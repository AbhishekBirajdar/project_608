...
from src import AutoApplyModel
from src.utils.utils import display_pdf, download_pdf, read_file, read_json
from src.utils.metrics import jaccard_similarity, overlap_coefficient, cosine_similarity
from src.variables import LLM_MAPPING
...
def encode_tex_file(file_path):
    try:
        ...
        file_paths = [file_path.replace('.pdf', '.tex'), os.path.join(current_loc, 'src', 'templates', 'resume.cls')]
...
try:
    st.header("Get :green[Job Aligned] :orange[Killer] Resume :sunglasses:", divider='rainbow')
    #st.header("green[Job Aligned] :orange[Personalized] Resumes", divider='rainbow')
    ...
    with col_url:
        is_url_button = st.toggle('Job Posting URL', False)
